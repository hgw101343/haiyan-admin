...
import { getDishes, createDish, updateDish, deleteDish, batchDeleteDishes, toggleDishAvailable, getCategories, uploadImage } from '../../api'
...
export default function DishesPage() {
  ...
  const fetchDishes = async (p = page) => {
    setLoading(true)
    try {
      const res: any = await getDishes({ page: p, pageSize: 10, keyword, categoryId: catFilter, recommended: recFilter })
      setDishes(res.data || [])
  ...
  const columns = [
    {
      ...
      render: (url: string) =>
        url ? (
          <Image src={url} width={56} height={56} style={{ objectFit: 'cover', borderRadius: 8 }} />
        ) : (
          <div style={{ width: 56, height: 56, background: '#f5f5f5', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🍽️</div>
        ),
    },
    ...
    },
    {
      ...
      dataIndex: 'price',
      width: 100,
      render: (v: number) => <span style={{ color: '#ff6b35', fontWeight: 600 }}>¥{Number(v).toFixed(2)}</span>,
    },
    ...
    },
    {
      ...
      width: 80,
      render: (v: boolean) => v ? <Tag color="gold">⭐推荐</Tag> : <Tag>普通</Tag>,
    },
    {
      title: '操作',
      width: 120,
  ...
  return (
    <div>
      ...
      <Row gutter={8} style={{ marginBottom: 16 }}>
        ...
        <Col flex="120px">
          <Select
            ...
            allowClear
            style={{ width: '100%' }}
            onChange={(v) => setRecFilter(v === undefined ? undefined : v === 'true')}
            options={[
              { value: 'true', label: '⭐ 推荐' },
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }} initialValues={{ isActive: true }}>
          <Row gutter={16}>
            <Col span={14}>
          ...
          <Row gutter={16}>
            ...
            <Col span={6}>
              <Form.Item name="isRecommended" label="设为推荐" valuePropName="checked">
                <Switch />
              </Form.Item>
            </Col>
          </Row>
