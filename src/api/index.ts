 ...
 */
export const getUsers = (params?: { page?: number; pageSize?: number; keyword?: string; role?: string }) =>
  request.get('/auth/admin/users', { params })
/** 管理员创建新用户（可指定角色、手机号、真实姓名等） */
export const createUser = (data: { nickname: string; password: string; role?: string; phone?: string; realName?: string }) =>
  request.post('/auth/admin/register', data)
/** 管理员编辑用户信息（可修改昵称、密码、角色、手机号等） */
export const updateUser = (id: number, data: { nickname?: string; password?: string; role?: string; phone?: string; realName?: string }) =>
...
export const getTheme = (params?: { userId?: number }) => request.get<any, { success: boolean; data: ThemeConfig }>('/theme', { params })
...
export const getFeedbackList = () => request.get<any, { success: boolean; data: Feedback[] }>('/feedback')
/** 标记反馈为已读（仅管理员） */
export const markFeedbackRead = (id: number) => request.put<any, { success: boolean }>(`/feedback/${id}`)
